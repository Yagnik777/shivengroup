...
import { 
  ...
  Clock, Layers, Star, Search, Filter 
} from 'lucide-react';
...
export default function CandidateList() {
  ...
  const filteredList = useMemo(() => {
    return list.filter(app => {
      ...
      const matchesRole = roleFilter === "All" || (app.role || app.jobRole) === roleFilter;
      const matchesExp = expFilter === "All" || (app.experience || "Fresher") === expFilter;
      const matchesCat = catFilter === "All" || (app.category || "General") === catFilter;
      const matchesType = typeFilter === "All" || (app.jobType || "Full-time") === typeFilter;
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <main className="flex-1 p-6 md:p-10 relative">
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Job Role & Category</th>
                    <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Experience / Type</th>
                  ...
                  {filteredList.map((app) => (
                    <tr key={app._id} className="hover:bg-slate-50 transition-all cursor-pointer" onClick={() => setSelectedUser(app)}>
                      ...
                      <td className="px-6 py-5">
                        ...
                        <p className="text-[10px] font-black text-indigo-500 uppercase">{app.category || "General"}</p>
                      </td>
                      <td className="px-6 py-5">
                      ...
                      <td className="px-6 py-5">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                          (app.status || 'Pending') === 'Approved' ? 'bg-emerald-100 text-emerald-600' : 
        ...
        {/* --- FULL DETAILS SLIDE-OVER (બાકીનો કોડ સેમ છે) --- */}
        {selectedUser && (
              ...
              <div className="p-8 space-y-10 pb-32">
                <div className="bg-gradient-to-br from-slate-900 to-indigo-950 rounded-[2.5rem] p-8 text-white shadow-2xl">
                  <div className="flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="text-center md:text-left">
                      ...
                      <p className="text-indigo-300 font-bold text-lg">{selectedUser.role || selectedUser.profession}</p>
                      <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-4">
                        <span className="bg-white/10 px-3 py-1 rounded-full text-[10px] font-bold border border-white/10 flex items-center gap-1 uppercase tracking-widest"><Layers size={12}/> {selectedUser.category || "General"}</span>
                        <span className="bg-white/10 px-3 py-1 rounded-full text-[10px] font-bold border border-white/10 flex items-center gap-1 uppercase tracking-widest"><Star size={12}/> {selectedUser.experience || "Fresher"}</span>
                        <span className="bg-white/10 px-3 py-1 rounded-full text-[10px] font-bold border border-white/10 flex items-center gap-1 uppercase tracking-widest"><Clock size={12}/> {selectedUser.jobType || "Full-time"}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="relative border-l-2 border-indigo-100 pl-8 space-y-2">
                    ...
                    <p className="text-indigo-600 font-black text-xs bg-indigo-50 px-2 py-1 rounded-md inline-block uppercase tracking-wider">{selectedUser.jobDepartment} • {selectedUser.jobIndustry}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      {selectedUser.github && <a href={selectedUser.github} target="_blank" className="flex items-center justify-between p-3 bg-slate-100 rounded-xl text-slate-700 text-xs font-black hover:bg-slate-200 transition-all"><span>GitHub</span> <ExternalLink size={14}/></a>}
                      {selectedUser.portfolio && <a href={selectedUser.portfolio} target="_blank" className="flex items-center justify-between p-3 bg-indigo-50 rounded-xl text-indigo-700 text-xs font-black hover:bg-indigo-100 transition-all"><span>Portfolio</span> <ExternalLink size={14}/></a>}
                    </div>
                  </div>
                </div>
