...
import { BadgeCheck, MapPin, Mail, Phone, Edit3, Save, Loader2, Briefcase, Star, X } from 'lucide-react';
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 lg:p-12 mt-16 lg:mt-0">
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 border border-slate-100 shadow-sm relative overflow-hidden">
            <div className="flex flex-col md:flex-row items-center md:items-start lg:items-center gap-6 md:gap-8 relative z-10">
              ...
              <div className="flex flex-col items-center gap-3 w-full md:w-auto">
                ...
                >
                  {isLoading ? <Loader2 className="animate-spin" size={16}/> : isEditing ? <><Save size={16} /> Save Changes</> : <><Edit3 size={16} /> Edit Profile</>}
                </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* WORK INFO CARD */}
            <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm space-y-6 transition-all hover:shadow-md">
            ...
            <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-slate-100 shadow-sm space-y-6 transition-all hover:shadow-md">
              ...
              <div className="space-y-5">
                <EditableField label="Email Address" icon={<Mail size={18}/>} name="email" value={profile.email} isEditing={isEditing} onChange={handleChange} />
...
// Editable Field Component with Responsive Fixes
function EditableField({ label, icon, name, value, isEditing, onChange }) {
  return (
    <div className="flex items-center gap-4 group">
      <div className="w-10 h-10 md:w-11 md:h-11 rounded-xl bg-slate-50 flex items-center justify-center text-indigo-600 border border-slate-100 transition-colors group-hover:bg-indigo-50">
        {icon}
      </div>
      <div className="flex-1 overflow-hidden">
        ...
        {isEditing ? (
          ...
          <p className="text-sm font-bold text-slate-700 truncate">{value || "Not Set"}</p>
        )}
      </div>
    </div>
  );
