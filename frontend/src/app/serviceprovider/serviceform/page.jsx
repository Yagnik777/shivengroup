...
import { PlusCircle, Loader2, CheckCircle2, Trash2, ChevronDown, ChevronUp, Pencil, IndianRupee, MessageCircle, Phone } from 'lucide-react';
...
export default function CreateService() {
  ...
  const fetchProviderDetails = async (email) => {
    try {
      // પ્રોફાઇલ API માંથી ડેટા ફેચ કરવો
      const res = await fetch(`/api/serviceprovider/register?email=${email}`);
      ...
      if (data.success && data.user) {
        ...
        // જો નવો સર્વિસ ફોર્મ હોય, તો પ્રોફાઇલનો WhatsApp Number ફેચ કરીને સેટ કરવો
        if (!editingId) {
          setFormData(prev => ({ 
            ...prev, 
